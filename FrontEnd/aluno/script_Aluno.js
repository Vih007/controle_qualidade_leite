...
btnEnviar.addEventListener('click', function (e) {
    ...
    function editarLote(button) {
    const row = button.closest('tr');
    const id = row.getAttribute('data-id');

    const dataCell = row.querySelector('.data-lote');
    const quantidadeCell = row.querySelector('.quantidade-lote');
    const tanqueCell = row.querySelector('.tanque-lote');

    const dataAtual = dataCell.textContent;
    const quantidadeAtual = quantidadeCell.textContent.replace(' L', '');
    const tanqueAtual = tanqueCell.textContent;

    dataCell.innerHTML = `<input type="date" class="input-edicao" value="${dataAtual}">`;
    quantidadeCell.innerHTML = `<input type="number" class="input-edicao" value="${quantidadeAtual}" step="0.01">`;
    tanqueCell.innerHTML = `<input type="text" class="input-edicao" value="${tanqueAtual}">`;
    ...
    button.onclick = function() {
        ...
        const novoTanque = tanqueCell.querySelector('input').value;

        salvarEdicaoLote(id, novaData, novaQuantidade, novoTanque, row, button);
    };
}

function salvarEdicaoLote(id, novaData, novaQuantidade, novoTanque, row, button) {
    fetch('../../BackEnd/editar_lote.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: `id_lote=${id}&data=${encodeURIComponent(novaData)}&quantidade_total=${encodeURIComponent(novaQuantidade)}&tanque=${encodeURIComponent(novoTanque)}`
    })
    .then(res => res.json())
    .then(data => {
        if (data.success) {
            row.querySelector('.data-lote').textContent = novaData;
            row.querySelector('.quantidade-lote').textContent = novaQuantidade + ' L';
            row.querySelector('.tanque-lote').textContent = novoTanque;

            button.textContent = 'Editar';
            button.classList.remove('btn-salvar');
            button.classList.add('btn-editar');
            button.onclick = function() { editarLote(button); };
        } else {
            alert('Erro ao atualizar lote: ' + data.message);
        }
    })
    .catch(error => alert('Erro na requisição: ' + error));
}

function excluirLote(id, button) {
    if (!confirm('Tem certeza que deseja excluir este lote?')) return;

    const row = button.closest('tr');

    fetch('../../BackEnd/excluir_lote.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: `id_lote=${id}`
    })
    ...
    .then(data => {
        if (data.success) {
            ...
            const mensagem = document.createElement('div');
            mensagem.className = 'mensagem-sucesso';
            mensagem.textContent = 'Lote excluído com sucesso!';
            document.body.appendChild(mensagem);
            setTimeout(() => mensagem.remove(), 3000);
        } else {
    ...
    .catch(error => alert('Erro ao conectar com o servidor: ' + error));
}
